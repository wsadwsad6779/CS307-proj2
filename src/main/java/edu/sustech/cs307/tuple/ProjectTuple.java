...
public class ProjectTuple extends Tuple {
    ...
    public Value getValue(TabCol tabCol) throws DBException {
        for (TabCol projectColumn : schema) {
            // 表名匹配：如果任一方的表名为空，则只匹配列名
            boolean tableMatch = projectColumn.getTableName().isEmpty()
                    || tabCol.getTableName().isEmpty()
                    || projectColumn.getTableName().equals(tabCol.getTableName());
            if (tableMatch && projectColumn.getColumnName().equals(tabCol.getColumnName())) {
